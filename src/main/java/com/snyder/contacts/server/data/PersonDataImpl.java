...
public class PersonDataImpl implements PersonData
{
	...
	public void insertSubtype(DSLContext context, Contact contact)
	{
		...
		context
		    .insertInto(PERSON, PERSON_ID, TITLE_CD, FIRST_NAME, MIDDLE_INITIAL, LAST_NAME)
		    .values(person.getId(), titleToCode(person.getTitle()), person.getFirstName(),
		        person.getMiddleInitial(), person.getLastName()).execute();
	}
	...
	public Person getSubtype(DSLContext context, int contactId)
	{
		Record5<Integer, Short, String, String, String> record =
		    context.select(PERSON_ID, TITLE_CD, FIRST_NAME, MIDDLE_INITIAL, LAST_NAME).from(PERSON)
		        .where(PERSON_ID.equal(contactId)).fetchAny();
