...
public class EmailDataImpl implements EmailData
{
	...
	private static final Field<String> EMAIL_ = org.jooq.h2.generated.tables.Email.EMAIL.EMAIL_;
	...
	public List<EmailAddress> getEmailAddressesForContactId(DSLContext context, int contactId)
	{
		Result<Record2<String, String>> results =
		    context.select(EMAIL_TYPE, EMAIL_).from(EMAIL).where(EMAIL_CONTACT_ID.equal(contactId))
		        .fetch();
	...
	{
		context.delete(EMAIL).where(EMAIL_CONTACT_ID.equal(contactId)).execute();
	}
