...
public class ContactsDataImpl extends DataImpl implements ContactsData
{
	...
	private static final Field<String> INFO = org.jooq.h2.generated.tables.Contact.CONTACT.INFO;
	...
	public int createContact(Contact contact)
	{
		...
		try
		{
			...
			ContactRecord inserted =
			    context.insertInto(CONTACT, INFO).values(contact.getInfo()).returning(CONTACT_ID)
			        .fetchOne();
	...
	public void deleteContact(int id)
	{
		...
		try
		{
			...
			context.delete(CONTACT).where(CONTACT_ID.equal(id)).execute();
	...
	public Contact getContactById(int id)
	{
		...
		try
		{
			...
			// This approach requires more queries than is strictly necessary when the correct
			// subtype
			// table is not queried first. This could be combined into a single query by joining
			// PERSON and BUSINESS to CONTACT and then creating the subtype object corresponding to
			// the
			// populated fields. Although this saves an occasionally-wasted query, it is much more
			// complex, difficult-to-maintain, and the join comes with its own performance hit. If
			// more
			...
			contact.getEmailAddresses()
			    .addAll(emailData.getEmailAddressesForContactId(context, id));

			contact.getPhoneNumbers()
			    .addAll(phoneNumberData.getPhoneNumbersForContact(context, id));
