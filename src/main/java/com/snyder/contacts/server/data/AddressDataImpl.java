...
public class AddressDataImpl implements AddressData
{
	...
	public List<Address> getAddressesForContact(DSLContext context, int contactId)
	{
		Result<AddressRecord> results =
		    context.selectFrom(ADDRESS).where(CONTACT_ID.equal(contactId)).fetch();
	...
	@Override
	public void insertAddressesForContact(DSLContext context, int contactId, List<Address> addresses)
	{
